...
#d <- decompose(ts(full_wind_data$Wind_Clean, frequency=12), type= "additive")
d <- decompose(clean_wind_data)
plot(d)
...
df = data.frame(cbind(full_wind_data$Date, ds, dt, dr))
...
ggplot(data.frame(Date = as.Date(wind_data$DATE), as.numeric(wind_trend)))
